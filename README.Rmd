---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "100%"
)
```

# hospitals <img src='man/figures/logo.svg' align="right" height="139" />

<!-- badges: start -->
<!-- badges: end -->

This R data package contains one single dataset --- the eponymous `hospitals` tibble --- comprising the set of hospitals in the Portuguese National Health Service.

## Installation

You can install the current version of *hospitals* with:

``` r
# install.packages("remotes")
remotes::install_github("nhs-pt/hospitals")
```

## The dataset

```{r dataset}
library(hospitals)

# hospitals
```

